...
$ bosh vendor-package ruby-2.4.6 ~/workspace/ruby-release
```

The above code will add `ruby-2.4.6` to `your-release` and introduce a `spec.lock`.

Included packages:

- ruby-2.4.6 which includes:
  - ruby-2.4.6
  - yaml-0.1.7
  - rubygems-2.7.9
...
```bash
#!/bin/bash -eu
source /var/vcap/packages/ruby-2.4.6/bosh/compile.env
...
bosh_bundle
...
```bash
#!/bin/bash -eu
source /var/vcap/packages/ruby-2.4.6/bosh/runtime.env
